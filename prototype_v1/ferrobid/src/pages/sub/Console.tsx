  ...
  return (
    <div>
      <SectionTitle
        ...
        right={<Badge tone="bg-violet-50 dark:bg-violet-400/10 text-violet-700 dark:text-violet-300 ring-violet-200 dark:ring-violet-400/25">{Object.values(perms).filter(Boolean).length} of {Object.keys(perms).length} permissions granted</Badge>}
      />
                ...
                return (
                  <Card key={a.id} className="flex items-center gap-3 p-4">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="text-[11px] font-bold text-faint">{a.id}</div>
                      <div className="truncate text-sm font-bold text-ink">{lot.title}</div>
                      <div className="text-xs text-faint">{inrCompact(a.currentBid)} · {a.bidderCount} bidders</div>
                    </div>
                    {a.status === 'live'
                      ? <Badge tone="bg-ember-50 dark:bg-ember-400/10 text-ember-700 dark:text-ember-300 ring-ember-200 dark:ring-ember-400/25"><Radio size={11} /> {timeLeft(a.endsAt, now)}</Badge>
                      : <Badge tone="bg-indigo-50 dark:bg-indigo-400/10 text-indigo-700 dark:text-indigo-300 ring-indigo-200 dark:ring-indigo-400/25"><Timer size={11} /> Scheduled</Badge>}
                  </Card>
                );
              })}
            </div>
          ...
          perms.manageLots || perms.manageCatalogue ? (
            <>
              <div className="relative mb-3 max-w-md">
                ...
                <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search catalogue…" className="w-full rounded-xl border border-line-strong bg-surface py-2 pl-9 pr-3 text-sm focus:outline-none focus:ring-2 focus:ring-steel-500/25" />
              </div>
                ...
                {lots.filter((l) => !q || (l.title + l.id).toLowerCase().includes(q.toLowerCase())).map((l) => (
                  <Card key={l.id} className="p-3.5">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="text-[10px] font-bold text-faint">{l.id}</div>
                        <div className="truncate text-xs font-bold text-ink">{l.title}</div>
                      </div>
                    </div>
                    <div className="mt-2.5 flex items-center justify-between">
                      <StatusBadge status={l.status} />
                      <span className="text-xs font-bold text-ink">{inrCompact(l.basePrice)}</span>
                    </div>
                    <div className={cx('mt-2 text-[10px] font-semibold', perms.manageLots ? 'text-emerald-600 dark:text-emerald-400' : 'text-faint')}>
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {users.filter((u) => !u.id.startsWith('b-')).map((u) => (
                <Card key={u.id} className="p-4">
                  ...
                  <div className="text-xs text-faint">{u.phone}</div>
                </Card>
              ))}
            </div>
