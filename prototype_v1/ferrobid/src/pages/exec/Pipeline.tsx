    ...
    <div>
      <SectionTitle
        ...
        right={pendingEntities > 0 && (
          <button onClick={() => nav('/exec/entities')} className="flex items-center gap-2 rounded-xl border border-ember-200 dark:border-ember-400/25 bg-ember-50 dark:bg-ember-400/10 px-3.5 py-2 text-xs font-bold text-ember-700 dark:text-ember-300 hover:bg-ember-100 cursor-pointer">
        ...
        {COLUMNS.map((col) => {
          ...
          return (
            <div key={col.title} className="w-60 shrink-0">
              <div className={cx('rounded-xl border-t-4 bg-line/50 p-2', col.tone)}>
                <div className="flex items-center justify-between px-1.5 pb-2 pt-1">
                  <span className="text-xs font-extrabold uppercase tracking-wide text-muted">{col.title}</span>
                  <span className="rounded-full bg-surface px-2 py-0.5 text-[10px] font-bold text-muted">{items.length}</span>
                </div>
                  ...
                  {items.map((l) => (
                    <Card key={l.id} onClick={() => nav(col.to)} className="p-2.5">
                      ...
                      <div className="mt-2 flex items-center justify-between">
                        <span className="text-[10px] text-faint">{l.quantity}</span>
                        <Badge tone="bg-surface-2 text-muted ring-line">{inrCompact(l.basePrice)}</Badge>
                      </div>
                      {l.status === 'live' && <div className="mt-1.5 flex items-center gap-1 text-[10px] font-bold text-ember-600 dark:text-ember-400"><span className="h-1.5 w-1.5 rounded-full bg-ember-500 animate-pulse" /> LIVE bidding</div>}
                      {l.status === 'rejected' && <div className="mt-1.5 text-[10px] font-bold text-red-500">Rejected</div>}
                    </Card>
                  ))}
                  {items.length === 0 && <div className="rounded-lg border border-dashed border-line-strong py-6 text-center text-[10px] text-faint">Empty</div>}
                </div>
              </div>
            </div>
          );
      ...
      <p className="mt-2 text-[11px] text-faint">Click any card to open the stage's work queue. Lots move automatically as you action them.</p>
    </div>
  );
}
