/* Auction status tracker — auction-wise. The index lists every auction you won lots
...
const STAGES: { key: AuctionStatusStage; label: string }[] = [
...
export default function AuctionStatus() {
    ...
    return (
      <Page>
        <EmptyState
          ...
          title="Sign in to track auction status"
  ...
  if (myDos.length === 0) {
    return (
      <Page>
        <PageHeader title="Auction status tracker" sub={INTRO} />
        <EmptyState
...
function AuctionCard({ cat, dos }: { cat: Catalogue; dos: DeliveryOrder[] }) {
  ...
  return (
    <Link to={`/buyer/auction-status?auction=${cat.id}`} className="card card-hover group p-4 flex flex-col gap-4">
      ...
      {/* Auction status progress — one segment per won lot, each filled by how far
      ...
      <div>
        <div className="flex items-baseline justify-between gap-2">
          <span className="text-[11px] font-semibold uppercase tracking-wider text-ink-faint">Auction status</span>
        ...
        <div className="mt-1.5 flex items-center gap-1" role="progressbar"
          ...
          aria-label={`Auction status ${s.pct}% complete across ${dos.length} lot${dos.length > 1 ? 's' : ''}`}>
          {dos.map((d) => {
