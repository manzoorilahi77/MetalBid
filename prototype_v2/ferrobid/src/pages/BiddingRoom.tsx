...
import {
  AmountGrid, AmountInput, Button, Chip, Countdown, EmptyState, Input, Modal, Segmented, StatusChip, Toggle, cx,
} from '../components/ui'
...
export default function BiddingRoom() {
  ...
  return (
    <Page className="pb-10">
              ...
              return (
                <button key={l.id}
                  ...
                  <div className="flex items-center justify-between gap-1.5">
                    ...
                    {lLeading
                      ? <Chip tone="success" className="h-4 text-[9px] px-1.5 shrink-0">H1</Chip>
                  ...
                  <div className="flex items-center justify-between gap-1.5 mt-1">
                    ...
                    <span className="text-[9px] font-semibold text-danger truncate">
                      {lLeading ? 'Hold/raise' : 'Bid or let go'}
                    </span>
                  </div>
                </button>
              )
      ...
      {/* step 1 — do you want to bid at all? only a "yes" here opens the amount confirmation */}
      <Modal open={preConfirmOpen} onClose={() => setPreConfirmOpen(false)} title="Place a bid?">
        <div className="space-y-4">
          <p className="text-sm text-ink-muted">
            Do you want to bid <b className="num text-ink">{inr(confirmRate)}/{lot.uom}</b> on <b className="num text-ink">{lot.lotNo}</b>?
          </p>
      ...
      {/* step 2 — bid confirmation modal, with the exact amount to sign off on */}
