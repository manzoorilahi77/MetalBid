    ...
    <div>
      ...
      {kycStatus !== 'verified' && (
        <Card className="mb-4 flex flex-wrap items-center gap-3 border-l-4 border-l-amber-400 p-4">
          ...
          {kycStatus === 'pending' && <Badge tone="bg-amber-50 text-amber-700 ring-amber-200"><Clock size={11} /> In review</Badge>}
        </Card>
      )}

      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <Stat label="Wallet balance" value={inrCompact(wallet.balance)} hint="Available to bid" icon={<Wallet size={18} />} accent="bg-emerald-50 text-emerald-600" />
        <Stat label="EMD locked" value={inrCompact(wallet.emdLocked)} hint="Auto-refunds if you lose" icon={<Lock size={18} />} accent="bg-amber-50 text-amber-600" />
        <Stat label="Active bids" value={`${myBidAuctions.size} lots`} hint={leading ? `Leading in ${leading}` : 'Place a bid to compete'} icon={<Gavel size={18} />} accent="bg-ember-50 text-ember-600" />
        <Stat label="Auctions won" value={won} hint="Track under Fulfilment" icon={<Trophy size={18} />} accent="bg-steel-50 text-steel-700" />
      </div>

      <div className="mt-6 grid gap-5 lg:grid-cols-3">
        <div className="lg:col-span-2">
          <div className="mb-3 flex items-center justify-between">
            <h2 className="flex items-center gap-2 text-sm font-bold text-steel-950">
              <span className="h-2 w-2 rounded-full bg-ember-500 animate-pulse" /> Live auctions
            </h2>
            <button onClick={() => nav('/browse')} className="text-xs font-bold text-steel-600 hover:underline cursor-pointer">Browse all →</button>
          </div>
              ...
              return (
                <Card key={a.id} onClick={() => nav(`/lot/${lot.id}`)} className="flex items-center gap-4 p-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="text-xs text-slate-400">{lot.quantity} · {lot.location}</div>
                    {mine && <Badge tone="bg-emerald-50 text-emerald-700 ring-emerald-200" className="mt-1">You're leading</Badge>}
                    {!mine && myBidAuctions.has(a.id) && <Badge tone="bg-red-50 text-red-600 ring-red-200" className="mt-1">Outbid — act now</Badge>}
                  </div>
                  <div className="text-right shrink-0">
                    <div className="text-sm font-extrabold text-steel-900">{inrCompact(a.currentBid)}</div>
                    <div className="flex items-center justify-end gap-1 text-[11px] font-semibold text-ember-600"><Timer size={11} /> {timeLeft(a.endsAt, now)}</div>
                  </div>
                </Card>
              );
            })}
        ...
        <div className="space-y-4">
          ...
          <Card className="p-4">
            <h3 className="text-sm font-bold text-steel-950">Quick actions</h3>
